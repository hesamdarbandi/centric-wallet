...
app.post("/send", (req, res) => {
  ...
  const { sender, recipient, amount, sign , hash } = req.body;

  const hashMessage = hashMessage(JSON.stringify({ sender, recipient, amount }));
  if(toHex(hashMessage) != hash){
      res.status(400).send({ message: "transfer not valid" });
  }

  var senderPrivateKey = balances[sender].privateKey;
  var signMessage = secp.secp256k1.sign(messageHash, privateKey);

  if(toHex(signMessage) != sign){
    res.status(400).send({ message: "transfer not valid" });
}
